...
class KR6control:
    ...
    def __init__(self, fc=100.0, ns='kr6'):
        ...
        self.home = np.array([-0.0* math.pi, -math.pi / 2, math.pi / 2, 0.0, -math.pi / 2, 0.0]).reshape((6, 1))
...
#         print self.kin.cart_inertia()
        ...
        self._cbFTSensor= rospy.Subscriber("/netft_data_172_1_10_1", WrenchStamped, self.callback_ft_sensor, queue_size=1) #ATI Axia80 real
    ...
    def callback_joint_states(self, data):
        #dt = (self._joint_states.header.stamp.secs + 1e-9*self._joint_states.header.stamp.nsecs) - (data.header.stamp.secs + 1e-9*data.header.stamp.nsecs)
        dt = 1.0/125.0
        ...
        for i in range(len(self._joint_states.name)):
            for j in range(len(self.joint_names)):
                if self._joint_states.name[i] == self.joint_names[j]:
                    #self.dq[j] = (self._joint_states.position[i]-self.q[j])/dt
...
#         return self.kin.jacobian() * vel_joints
    ...
    def publish_data(self):
        ...
        self.robot_pose.pose.position.z = 2.0
        self.robot_pose.pose.orientation.x = 1.0
        self.robot_pose.pose.orientation.y = 0.0
        self.robot_pose.pose.orientation.z = 0.0
        self.robot_pose.pose.orientation.w = 0.0
        self.robot_pose.header = Header(stamp=rospy.Time.now(), frame_id='base', seq=self.iter)
        
        self.object_pose.pose.position.x = 0.0
        self.object_pose.pose.position.y = 0.0
        self.object_pose.pose.position.z = 1.0
    ...
    def loop(self, fc=100.0):
        ...
        while not rospy.is_shutdown():
            ...
            u = self.w * self.Tc + self.u
 
            joint_limit = False
            for i in range(0, len(u)):
 	            if u[i] < self.uMin[i] or u[i] > self.uMax[i] :
 	                joint_limit = True
